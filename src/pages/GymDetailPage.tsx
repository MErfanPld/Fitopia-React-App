  ...
  const handleAddComment = async () => {
    ...
    try {
      ...
      if (created) {
        ...
        setNewComment("");
        setToast({
          message: "نظر نمایش داده شد (API ثبت عمومی در دسترس نبود)",
          type: "warning",
        });
      }
    } finally {
      ...
      <main className="relative z-10 home-shell home-pad pb-[calc(6.75rem+env(safe-area-inset-bottom))] md:pb-12">
        <div className="mx-auto flex w-full max-w-3xl flex-col gap-4 sm:gap-5 lg:max-w-4xl">
          ...
          {Array.isArray(gym.videos) && gym.videos.length > 0 ? (
            <Card>
              <div className="mb-3">
                <SectionTitle>ویدیوها</SectionTitle>
              </div>
              <div className="flex flex-col gap-3">
                {gym.videos.map((vid: any, idx: number) => {
                  const raw = typeof vid === "string" ? vid : (vid.video_url || vid.url || "");
                  const src = resolveMedia(raw) || raw;
                  const yt = src ? getYoutubeEmbed(src) : null;
                  const title = typeof vid === "object" ? vid.title : undefined;
                  return (
                    <div key={typeof vid === "object" ? (vid.id ?? idx) : idx} className="overflow-hidden rounded-xl border border-white/10 bg-black/40">
                      {title ? (
                        <p className="px-3 pt-2 text-xs font-bold text-white/80 flex items-center gap-1.5">
                          <Play size={12} className="text-primary" aria-hidden />
                          {title}
                        </p>
                      ) : null}
                      {yt ? (
          ...
          <Card>
            ...
            <div className="mb-3 space-y-2">
              <div className="flex items-center justify-end gap-1" role="group" aria-label="امتیاز">
                {[1, 2, 3, 4, 5].map((n) => (
                  <button
                    key={n}
                    type="button"
