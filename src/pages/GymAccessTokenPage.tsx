    ...
    <div className="min-h-dvh bg-[#07070A] text-right home-with-rail">
      ...
      <main className="relative z-10 home-shell home-pad pb-[calc(6.75rem+env(safe-area-inset-bottom))] md:pb-12">
        <div className="mx-auto w-full max-w-md sm:max-w-lg md:max-w-xl flex flex-col gap-4">
          ...
          {!loading && subscriptionInfo ? (
            <section className="rounded-2xl border border-white/[0.08] bg-[#121216] p-4">
              <div className="flex items-center justify-between gap-2 mb-3">
                ...
                <span className="text-[11px] text-white/45">{accessibleGyms.length > 0 ? `${formatPersianNumber(accessibleGyms.length)} باشگاه در اشتراک شما` : "—"}</span>
              </div>
          ...
          {!loading && pastTokens.length > 0 ? (
            <section>
              <h3 className="text-sm font-bold text-white mb-2.5">بلیت‌های قبلی</h3>
              <ul className="space-y-2">
                {pastTokens.map((t) => {
                  const meta = statusMeta(t.status);
                  const Icon = meta.Icon;
                  return (
                    <li key={t.id} className="flex items-center gap-3 rounded-xl border border-white/[0.06] bg-[#121216] px-3 py-2.5">
                      <Icon size={16} className="shrink-0 text-white/40" aria-hidden />
                      <div className="min-w-0 flex-1">
                        <code className="text-xs font-mono text-white/70" dir="ltr">{t.token_code}</code>
      ...
      {showQRModal && activeToken ? (
        <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center" role="dialog" aria-modal="true">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-[2px]" onClick={() => setShowQRModal(false)} aria-hidden />
          <div className="relative z-10 w-full max-w-sm rounded-t-3xl sm:rounded-3xl border border-white/10 bg-[#121216] p-5 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <button type="button" onClick={() => setShowQRModal(false)} className="inline-flex h-10 w-10 items-center justify-center rounded-xl text-white/70" aria-label="بستن"><X size={20} aria-hidden /></button>
              <h3 className="text-base font-bold text-white">QR بلیت</h3>
              <span className="w-10" aria-hidden />
            </div>
            {activeToken.qr_code ? (
              <img src={activeToken.qr_code} alt="QR" className="mx-auto w-48 h-48 rounded-xl bg-white p-2" />
            ) : (
              <p className="text-center text-sm text-white/50 py-8">QR در دسترس نیست</p>
            )}
            <p className="mt-3 text-center font-mono text-sm text-white/80" dir="ltr">{activeToken.token_code}</p>
          </div>
        </div>
      ) : null}
      {showCopyModal && activeToken ? (
        <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center" role="dialog" aria-modal="true">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-[2px]" onClick={() => setShowCopyModal(false)} aria-hidden />
          <div className="relative z-10 w-full max-w-sm rounded-t-3xl sm:rounded-3xl border border-white/10 bg-[#121216] p-5 shadow-2xl space-y-4">
            <div className="flex items-center justify-between">
              <button type="button" onClick={() => setShowCopyModal(false)} className="inline-flex h-10 w-10 items-center justify-center rounded-xl text-white/70" aria-label="بستن"><X size={20} aria-hidden /></button>
              <h3 className="text-base font-bold text-white">کپی کد بلیت</h3>
              <span className="w-10" aria-hidden />
            </div>
            <code className="block rounded-xl bg-black/40 border border-white/10 px-3 py-3 text-center font-mono text-sm text-white" dir="ltr">{activeToken.token_code}</code>
            <button type="button" onClick={copyToClipboard} className="btn btn-primary w-full min-h-12 text-sm font-bold inline-flex items-center justify-center gap-2">
              {copyFeedback ? (<><CheckCircle2 size={18} aria-hidden /> کپی شد</>) : (<><Copy size={18} aria-hidden /> کپی</>)}
            </button>
          </div>
        </div>
      ) : null}
